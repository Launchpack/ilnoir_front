...
export default {
  ...
  methods: {
    ...
    calcOrderProductDiscountPrice(item) {
      ...
      for(let i=0; i<orderproducts.length; i++) {
        ...
        if(price.discount_type===0) {
          item.discount_price = price.price * (price.discount_rate / 100)
        }
        if(item.order_product_option.length>0) {
          for(let j=0; j<item.order_product_option.length; j++) {
            let option = item.order_product_option[j];
    ...
    calcTotal() {
      ...
      for(let i=0; i<this.value.order_product.length; i++) {
        ...
        total_delivery_cost += op.op_delivery_price;
      }

      this.value.order.comm_price = total_comm_price;
      this.value.order.point = total_point;
      ...
      this.value.order.total_price = total_product_price + total_delivery_cost - total_discount_price;
