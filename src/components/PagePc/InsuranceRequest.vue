    ...
    <div :style="pcMaxWidth">
      ...
      <div class="flex-end size-14">
        ...
        <div class="btn-back text-center weight-400 unselect" @click="clickBtn"
             style="margin-left:12px">다른 이름으로 조회
        </div>
      </div>
        ...
        <div class="size-15" style="border-top:2px solid #696969">
          <div class="table-contents flex-between">
            ...
            <div v-if="params['privacy']" class="flex-align size-13"
              ...
              <div class="btn-delete unselect weight-400" style="margin-left:12px"
                   @click="clickDelete('privacy')">삭제
              </div>
            </div>
            <div v-else class="size-13 flex-align">
              <a class="btn-download unselect weight-400"
                 href="https://ilnoir.s3.ap-northeast-2.amazonaws.com/claim/static/form/privacy240318.docx"
                 download
                 target="_blank"
          ...
          <div class="table-contents flex-between">
            ...
            <div v-if="params['identification']" class="flex-align size-13"
              ...
              <div class="btn-delete unselect weight-400" style="margin-left:12px"
                   @click="clickDelete('identification')">삭제
              </div>
            </div>
            <div v-else class="size-13 flex-align">
          ...
          <div class="table-contents flex-between">
            ...
            <div v-if="params['bank_book']" class="flex-align size-13"
              ...
              <div class="btn-delete unselect weight-400" style="margin-left:12px"
                   @click="clickDelete('bank_book')">삭제
              </div>
            </div>
            <div v-else class="size-13 flex-align">
          ...
          <div class="table-contents flex-between">
            <div class="flex-align">
              ...
              <div class="size-15 weight-400" style="margin-right:20px">유니패스 조회 화면 캡처</div>
              <div class="size-14 weight-400" style="color:#9e9e9e">수입화물진행정보 창에서 상위 첫번째 테이블만 캡처 첨부 </div>
            </div>
        ...
        <div class="size-15" style="border-top:2px solid #696969">
          <div class="table-contents flex-between">
            <div class="flex-align">
              ...
              <div class="size-12 weight-400" style="color:#bbbbbb">파손물품사진 / 수리비내역서 (파손품 수리시) / 화물수령확인서 (수리불가로 전손시)
              </div>
            </div>
          ...
          <div class="table-contents flex-between">
            ...
            <div v-if="params['misdelivery']" class="flex-align size-13"
              ...
              <div class="btn-delete unselect weight-400" style="margin-left:12px"
                   @click="clickDelete('misdelivery')">삭제
              </div>
            </div>
            <div v-else class="size-13 flex-align">
          ...
          <div class="table-contents flex-between">
            ...
            <div v-if="params['theft']" class="flex-align size-13"
              ...
              <div class="btn-delete unselect weight-400" style="margin-left:12px"
                   @click="clickDelete('theft')">삭제
              </div>
            </div>
            <div v-else class="size-13 flex-align">
              <a class="btn-download unselect weight-400"
                 v-if="acciState('theft')"
                 href="https://ilnoir.s3.ap-northeast-2.amazonaws.com/claim/static/form/theft240318.docx"
                 download
                 target="_blank"
          ...
          <div class="table-contents flex-between">
            ...
            <div v-if="params['lost']" class="flex-align size-13"
              ...
              <div class="btn-delete unselect weight-400" style="margin-left:12px"
                   @click="clickDelete('lost')">삭제
              </div>
            </div>
            <div v-else class="size-13 flex-align">
              <a class="btn-download unselect weight-400"
                 v-if="acciState('lost')"
                 href="https://ilnoir.s3.ap-northeast-2.amazonaws.com/claim/static/form/lost240318.docx"
                 download
                 target="_blank"
        ...
        <div class="size-15" style="border-top:2px solid #696969">
          ...
          <div v-if="dropState" class="drop-box size-14">
            <p>주식회사 일루와디지털랩 (이하 &quot;회사&quot;라 함)은 아래와 같이 개인정보취급방침을 정하여 이용자 권익 보호에 최선을 다하고 있습니다. 본 개인정보취급방침은 당사가 본
              웹사이트(http://ilnoir.us)에서 이용자의 정보를 수집 및 활용하는 방법과 범위를 알려 드리기 위한 내용을 담고 있습니다. 본 웹사이트를 이용하시거나 개인정보를 제출하시기 전에
              아래 내용을 먼저 확인 바랍니다.<br>
            </p>
            <div><strong><span>[</span></strong><strong><span>수집하는 개인 정보의 항목 및 수집 방법<span>]</span></span></strong></div>
            <div><span><span>&nbsp;</span>1. 회사는 보험가입 회원의 보험금 청구 서비스 제공<span>, </span>원활한 고객상담을 위해 이용자의 동의 하에 아래와 같은 정보를 <span>API</span>의 방법으로 필수항목으로 수집하고 있습니다<span>.&nbsp;</span></span>
            </div>
            <div><span><span>&nbsp;</span>2. </span><span>회사가 제공하는 클레임 서비스를 이용하는 고객</span></div>
            <div>
              <span>&nbsp; &nbsp; - </span><span>성명<span>(</span>국문<span>, </span>영문<span>), </span>이메일<span>, </span>휴대폰번호<span>, </span>신분증사본<span>, </span>보험금 수령 계좌정보<span>, </span>보험 가입시 제공 동의한 보험청약정보</span>
            </div>
            <div>
            ...
            <div>
              <span>&nbsp; &nbsp;- </span><span>이용하신 플랫폼에서 보험계약가입시 회사로 정보제공을 동의하신 내용<span>(</span>보험청약정보<span>)</span></span>
            </div>
            ...
            <div>
              <span>&nbsp; &nbsp;</span><span>보험가입에 따른 중요 가입안내 보험계약사실 내용 전달<span>, </span>보험금 청구 시 원활한 청구 서비스 제공</span>
            </div>
            <p><span>&nbsp;</span></p>
            <div><span>▶개인정보 수집의 동의</span></div>
            <div><span>&nbsp;</span>1. 회사는 귀하의 개인정보 수집에 대한 동의를 받고 있습니다<span>. </span>귀하의 개인정보 수집과 관련하여 본 웹사이트의 개인정보취급방침
              또는 이용약관의 내용에 대한 『동의』버튼 또는 『동의 안함』 버튼을 클릭할 수 있는 절차를 마련하고 있으며<span>, </span>귀하가 『동의』버튼을 클릭하면 개인정보 수집에 대해 동의한
              것으로 봅니다<span>.</span></div>
            <p><span>&nbsp;</span></p>
            ...
            <p><span>&nbsp;</span></p>
            <div><span>▶개인정보 자동수집 장치<span>(</span>쿠키<span>-cookie)</span>의 설치<span>, </span>운영 및 그 거부에 관한 사항</span>
            </div>
            <div><span>&nbsp;</span>1. 회사는 개인화되고 맞춤화된 서비스를 제공하기 위해서 이용자의 정보를 저장하고 수시로 불러오는<span> ‘</span>쿠키<span>(cookie)’ </span>등을
              운용합니다<span>. </span>쿠키란 회사의 웹사이트를 운영하는데 이용되는 서버가 이용자의 브라우저에 보내는 아주 작은 텍스트 파일로서 이용자의 컴퓨터 하드디스크에 저장됩니다.
            </div>
            <p><span>&nbsp;</span></p>
            <div><span>▶ 쿠키 등 사용 목적</span></div>
            <div><span>&nbsp;</span>1. 회원과 비회원의 접속 빈도나 방문 시간 등을 분석<span>, </span>이용자의 취향과 관심분야를 파악 및 자취
              추적<span>, </span>각종</div>
            <div><span>이벤트 참여 정도 및 방문 회수 파악 등을 통한 타겟 마케팅 및 개인 맞춤 서비스 제공</span></div>
            <div><span>&nbsp;</span>2. 귀하는 쿠키 설치에 대한 선택권을 가지고 있습니다<span>. </span>따라서<span>, </span>귀하는 웹 브라우저에서 옵션을
              설정함으로써 모든 쿠키를 허용하거나<span>, </span>쿠키가 저장될 때마다 확인을 거치거나<span>, </span>아니면 모든 쿠키의 저장을 거부할 수도
              있습니다<span>.</span></div>
            <div><span>▶ 쿠키 설정 거부 방법</span></div>
            <div><span>&nbsp;</span>1. 쿠키 설정을 거부하는 방법으로는 이용자가 사용하시는 웹 브라우저의 옵션을 택함으로써 모든 쿠키를 허용하거나 쿠키를 저장할 때마다 확인을
              거치거나<span>, </span>모든 쿠키의 저장을 거부할 수 있습니다<span>. </span>단<span>, </span>이용자께서 쿠키 설치를 거부하였을 경우 서비스 제공에 어려움이
              있을 수 있습니다<span>.</span></div>
            <div><span>&nbsp;</span>2. 설정방법 예<span>(</span>인터넷 익스플로어의 경우<span>) : </span>웹 브라우저 상단의
              도구<span> &gt; </span>인터넷 옵션<span> &gt; </span>개인정보 </div>
            ...
            <div><span>&nbsp;</span>1. 회사는 이용자들의 개인정보를 ‘개인정보의 수집목적’에서 고지한 범위내에서 사용하며<span>, </span>이용자의 사전 동의 없이는 동 범위를
              초과하여 이용하거나 원칙적으로 이용자의 개인정보를 외부에 공개하지 않습니다<span>. </span>다만<span>, </span>이용자들이 사전에 공개에 동의 경우 및 법령의 규정에
              의거하거나<span>, </span>수사 목적으로 법령에 정해진 절차와 방법에 따라 수사기관의 요구가 있는 경우는 예외로 합니다<span>.</span></div>
            ...
            <p><span>&nbsp;</span>3. 회원은 개인정보의 제<span>3</span>자 제공에 대하여 동의를 하지 않을 수 있고<span>, </span>언제든지
              제<span>3</span>자 제공 동의를 철회할 수 있습니다<span>. </span>동의를 거부하시는 경우에 제<span>3</span>자 제공에 기반한 관련 서비스의 이용제공이 제한될
              수 있습니다<span>.</span></p>
            <div><span>[</span><span>개인정보의 보호<span>]</span></span></div>
            <div><span>회사는 이용자의 개인정보의 취급에 있어 분실<span>, </span>도난<span>, </span>누출<span>, </span>변조 또는 훼손이 일어나지 않도록 각종 기술적<span>/</span>관리적 대책을 강구하고 있습니다<span>. </span>다만<span>, </span>비밀번호 등 주요 개인정보를 이용자 스스로가 관리를 허술하게 하여 분실<span>, </span>도난<span>, </span>누출시켰을 경우 그 책임은 이용자에게 있으므로<span>, </span>관리에 유의하시기 바랍니다<span>.</span></span>
            </div>
            <p><span>&nbsp;</span></p>
    ...
    <div class="position-fixed flex-center" :style="modalStyle" v-if="commonInvalid">
      <div class="text-center">
        ...
        <div class="modal-btn size-16 unselect"
             @click.stop.prevent="clickModal">확인
        </div>
      </div>
    </div>
