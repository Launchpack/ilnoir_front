<template>
<div style="padding: 12px;margin-bottom:12px;border-radius:8px" :style="[brandColor.main, brandBgColor.white]">
  ...
  <div class="flex-between">
    ...
    <!--별점, 작성자, 작성일자, 옵션-->
    <div>
        <rating :value="card.rate ? card.rate : 0" :rateSize="10" :disabled="true"
      ...
      <div class="flex-end">
        <div style="width:20px;height:20px;border-radius:50%" :style="brandBgColor.white">
          <icon-context-menu
              v-if="isMine"
              :iconStyle="[brandColor.main, {fontSize:'20px'}]"
              :menu="menu"
              @modify="modify"
              @del="del"
  ...
  <div class="grid-box" style="margin-top:8px">
    <div :style="brandColor.main">
      <div v-if="!card.secret">
        <div class="size-14 weight-500" style="white-space:pre-wrap; line-height:18px;">{{ card.title }}</div>
      </div>
      <div v-if="!card.secret">
        <div class="size-12" style="white-space:pre-wrap; line-height:16px;">{{ card.content }}</div>
      </div>
      <div v-else
           class="flex-align size-18 secret"
    ...
    <!-- 답변 -->
    <div style="margin: 16px 0 0 20px;padding:12px 12px 16px;border-radius:8px" 
      :style="brandBgColor.main" v-if="card.comment">
      <div class="flex-between margin-bottom-14">
        <div class="flex-align">
          <!--<div class="author-img" style="margin-right:8px"-->
          <!--:style="bgCoverStyle(card.author_img)"></div>-->
          <div class="size-12">
            <div :style="brandColor.primary">{{ card.answerer }}</div>
            <div :style="brandColor.sub">{{ dateFormat(card.comment_date, 'ago') }}</div>
          </div>
        </div>
      </div>
      <div v-if="!card.secret">
        <e-editor :html="card.comment" class="size-12" :style="brandColor.main"></e-editor>
      </div>
      <div v-else
            class="flex-align size-13 secret margin-bottom-8"
            :style="brandColor.help">
...
<script>
import LabelUserGrade from "@/components/LaunchPack_Common/Common/LabelUserGrade"
...
export default {
  ...
  methods: {
    ...
    modify() {
        let url = `review_reg?product_id=${this.card.product_id}&review_id=${this.card.id}`
        if(this.user.partner_id) 
          url = url + `&partner_id=${this.user.partner_id}`;
        if(this.user.user_id === Number(this.card.author_id)) 
          url = url + `&author_id=${this.card.author_id}`
        this.routerPush(url);
      },
    del() {
