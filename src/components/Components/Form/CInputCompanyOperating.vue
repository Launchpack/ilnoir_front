...
export default {
  ...
  methods: {
    ...
    /*getData(){
        for(var i=0; i<this.bsnhour.length; i++) {
          var hours = '';
          var brk = '';
          ...
          if (this.bsnhour[i].is_holiday) hours = '휴무';
            ...
            var bsn_st = this.bsnhour[i].start_time;
            var bsn_end = this.bsnhour[i].end_time;
            hours = '영업시간: ' + bsn_st + ' ~ ' + bsn_end;

            if (this.bsnhour[i].unable_break === false) {
              var brk_st = this.input_bsnhour.break_start;
              var brk_end = this.input_bsnhour.break_end;
