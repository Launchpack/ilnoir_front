...
<div :style="[pagePadding]" style="paddingBottom:32px; width:240px;">
  ...
  <div class="flex-justify wrap" style="padding:32px 0">
    ...
    <div class="size-12 weight-500 full-width text-center" style="padding-top:8px;">{{ user.name }}</div>
    <div class="full-width flex-center" style="padding-top:16px;">
      <btn-pill title="내 정보 수정" @clicked="clickProfile" bg="main" color="main" :fontSize="12"
        style="margin: 0 8px;"></btn-pill>
      <btn-pill title="로그아웃" @clicked="clickLogout" bg="main" color="main" :fontSize="12"
        style="margin: 0 8px;"></btn-pill>
    </div>
  </div>
  ...
  <div style="margin-bottom: 32px;">
    <template v-for="myitem in mymenu">
      <a v-if="myitem.link_type === 'outlink'" :href="myitem.outlink" target="_blank">
        <div :key="myitem.link"
          class="flex-between size-12 weight-500 menu-item unselect" 
          :style="[brandColor.main, {borderBottom: brandBorder.main.border}]">
          <div>{{myitem.name}}</div>
          <i class="material-icons size-18">{{myitem.icon}}</i>
        </div>  
      </a>
      <div :key="myitem.link" v-else
        class="flex-between size-12 weight-500 menu-item unselect" 
        :style="[brandColor.main, {borderBottom: brandBorder.main.border}]"
        @click="clickMenu(myitem)">
        <div>{{myitem.name}}</div>
        <i class="material-icons size-18">{{myitem.icon}}</i>
      </div>
    </template>
